...
export default class SampleDetails extends React.Component {
  ...
  sampleNumericInput(sample, unit, ref = '') {
  ...
  sampleAmount(sample) {
    if(sample.isMethodDisabled('amount_value') == false) {
      if(sample.isMethodRestricted('molecule') == true) {
        ...
        return (
          <table>
            <tbody>
              <tr>
                {this.sampleNumericInput(sample, 'mg', 'massMgInput')}

                {this.sampleNumericInput(sample, 'ml')}

                {this.sampleNumericInput(sample, 'mmol', 'amountInput')}
      ...
      return (
        <Input type="text" label="Amount" disabled value="***" readOnly/>
      )
    }
  }
