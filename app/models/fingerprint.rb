class Fingerprint < ActiveRecord::Base
  ...
  has_many :molecules
  # has_many :samples, through: :molecule
  ...
  scope :by_tanimoto_coefficient, -> (fp_vector,
    ...
    scope = where('num_set_bits >= ?', (threshold * query_num_set_bits).floor)
    ...
    query = sanitize_sql_for_conditions(["id,
      ...
      query_num_set_bits])
    tanimoto = unscoped.from("(#{common_set_bits.to_sql}) AS common_bits")
                       .select(query).sort { |x,y| x.tanimoto > y.tanimoto }

    fp_ids = tanimoto.map(&:id)
